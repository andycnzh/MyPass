...
namespace MyPass
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        private static async System.Threading.Tasks.Task SavePassInfo(string jsonPassInfo)
        {
            StorageFolder localFolder = ApplicationData.Current.LocalFolder;
            StorageFile textFile = await localFolder.CreateFileAsync("passInfo.txt", CreationCollisionOption.ReplaceExisting);

            using (IRandomAccessStream textStream = await textFile.OpenAsync(FileAccessMode.ReadWrite))
            {
                using (DataWriter textWriter = new DataWriter(textStream))
                {
                    textWriter.WriteString(jsonPassInfo);
